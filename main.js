...
const gcrevoke = ["917994206751-1621430788@g.us"]
prefix = '#'
shp = '⬡'
owner = "917994206751@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Subscribe 𝙍𝘼𝙃𝙀𝙀𝙎", "Chrome", "3.0"];
  ...
  client.on("qr", () => {
    console.log(
      ...
      client.sendMessage(`917994206751@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Connected Successfully!_`, MessageType.extendedText)
  ...
  client.on("open", () => {
    success("2", "Successfully connected to wa, Subscribe 𝙍𝘼𝙃𝙀𝙀𝙎");
console.log(color(figlet.textSync(`${spc1}            𝙍𝘼𝙃𝙀𝙀𝙎`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝐈 𝐖𝐑𝐎𝐓𝐄 𝐓𝐇𝐈𝐒 𝐒𝐂𝐑𝐈𝐏𝐓 𝐌𝐘𝐒𝐄𝐋𝐅!\n𝐍𝐎𝐓𝐄: 𝐃𝐎𝐍𝐓 𝐅𝐎𝐑𝐆𝐄𝐓 𝐓𝐎 𝐒𝐔𝐁𝐒𝐂𝐑𝐈𝐁𝐄 𝐏𝐄𝐏𝐄 𝐒𝐈𝐑', 'yellow'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/917994206751'))
    console.log(color('[𝙍𝘼𝙃𝙀𝙀𝙎]'), color('𝙍𝘼𝙃𝙀𝙀𝙎 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 client.sendMessage(`917994206751@s.whatsapp.net`, `*Hi Owner 𝙍𝘼𝙃𝙀𝙀𝙎😌❤️, the bot has been successfully connected to this number*\n╭────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n╰────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer pepe Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/917994206751?text=Hello bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("917994206751@s.whatsapp.net", `╭─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n╰────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Pepe Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/917994206751?text=Hello bro"}}})
...
fkontakk = { key: {
  fromMe: false,
    participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '917994206751-1604595598@g.us' } : {})
},
 message: {
    "contactMessage":{"displayName":'Pepe',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Xeon;;;\nFN:resku\nitem1.TEL;waid=917994206751:+91 690-9137-213\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hello everyone, am 𝙍𝘼𝙃𝙀𝙀𝙎 Bot😌❤️, ready to help you here 💖", "conversation");
      }
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
			 ...
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                ...
                welcomeBut = [{buttonId:`getdeskgc`,buttonText:{displayText:'GROUP DESCRIPTION'},type:1}, {buttonId:`git`,buttonText:{displayText:'GIT LINK'},type:1}]
